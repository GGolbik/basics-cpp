...
Client::Client(std::string serverAddress, unsigned short port) : enabled{false}, serverAddress{serverAddress}, port{port} {}
 ...
 * 
 * struct hostent *gethostbyname(const char *name); // Returns a pointer to a resultant struct hostent or success, or NULL on error. On error, the
       h_errno variable holds an error number. 
 * 
 * 
 ...
 *
 * ::gethostbyname(): If name is an IPv4 address, no lookup is performed and gethostbyname() simply copies name into the h_name field.
 * Returns the hostent structure or a null pointer if an error occurs.
 * 
 * :: bcopy() copies n bytes from src to dest.  The result is correct, even when both areas overlap.
 * 
 * The method call
 ...
 */
static bool createSocketAddress(const std::string &serverAddress, unsigned short port, sockaddr_in &socketAddress)
{
  ...
  else
  {
    ...
    // h_length: This is the length, in bytes, of each address.
...
static bool connect(int socket, sockaddr_in &socketAddress)
{
  ...
  return ::connect(socket, reinterpret_cast<sockaddr *>(&socketAddress), sizeof(sockaddr_in)) == 0;
}
...
bool Client::readString(std::string &message)
{
  ...
  } while (this->enabled && (errno == EAGAIN || errno == EWOULDBLOCK));
  return false;
}
