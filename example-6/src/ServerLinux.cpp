...
Server::Server(unsigned short port, std::string interfaceAddress) : port{port}, interfaceAddress{interfaceAddress}, enabled{false}, running{false}, listenSocket{-1} {}
 ...
 * 
 * ::inet_addr() interprets a string that represents numbers in the Internet standard dotted decimal notation and returns a corresponding Internet address.
 * If ::inet_addr() is successful, it returns the address in network byte order. Otherwise, it returns INADDR_NONE ( 0xFFFFFFFF ), and sets errno to indicate the type of error. 
 *
 * ::htons() converts the unsigned short integer hostshort from host byte order to network byte order.
 * 
 * ::htonl() converts the unsigned integer hostlong from host byte order to network byte order. Required to convert the macros INADDR_ANY, INADDR_LOOPBACK, INADDR_ANY.
 *
 * The method call
 ...
 * 
 * The <sys/socket.h> header defines the following macros, with distinct integer values:
 * - AF_INET
 * - AF_INET6
 * 
 ...
 *   };
 *  
 * Notice: errno is thread safe. On Linux, the global errno variable is thread-specific. POSIX requires that errno be threadsafe.
 *
 * @param port the port number
 * @param interfaceAddress the IP address of the interface.
 * @param socketAddress the socket address object.
 * @return true if socket address has been created. false if the method failed to create the socket address. On error you should check the errno value.
 */
static bool createSocketAddress(unsigned short port, const std::string &interfaceAddress, sockaddr_in &socketAddress)
{
 ...
 * Unlike pipes sockets support communication between unrelated processes, and even between processes running on different machines that communicate over a network.
 * Sockets are the primary means of communicating with other machines; telnet, rlogin, ftp, talk and the other familiar network programs use sockets.
 * 
 ...
 *   SOCK_STREAM : Stream Sockets - Stream sockets are reliable two-way connected communication streams.
 *   SOCK_DGRAM : Datagram Sockets - Datagram sockets are sometimes called "connectionless sockets"
 ...
 * The protocol determines what low-level mechanism is used to transmit and receive data.
 * Each protocol is valid for a particular namespace and communication style.
 * 
 * For each combination of style and namespace there is a default protocol, which you can request by specifying 0 as the protocol number.
 * So you would see sometime
 ...
 * - AF_INET
 * 
 * @param socketAddress The address with the specified address family / namespace.
 ...
 * 
 * @return true on success. On error, false is returned, and errno is set appropriately.
 */
...
/**
 * Allows socket descriptor to be reuseable (Forecefully attaching socket to the port).
 * 
 ...
 * It is useful if your server has been shut down, and then restarted right away while sockets are still active on its port.
 ...
 * 
 * SO_REUSEPORT: SO_REUSEADDR socket option already allows multiple UDP sockets to be bound to, and accept datagrams on, the same UDP port.
 * However, by contrast with SO_REUSEPORT, SO_REUSEADDR does not prevent port hijacking and does not distribute datagrams evenly across the receiving threads.  
 * 
 * You can find a way more detailed explanation of SO_REUSEADDR and SO_REUSEPORT at 
...
static bool setSocketOptions(int socket)
{
  ...
  if (::setsockopt(socket, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &enable, sizeof(int)) != 0)
  {
    return false;
  }
  return true;
 ...
 * 
 * By default, TCP sockets are in "blocking" mode. For example, when you call ::read() to read from a stream, control isn't returned to your program until at least one byte of data is read from the remote site.
 * This process of waiting for data to appear is referred to as "blocking".
 * The same is true for the write() API, the connect() API, etc.
 ...
 * This is an invaluable tool if you need to switch between many different connected sockets, and want to ensure that none of them cause the program to "lock up."
 * 
 * If you call ::read() in non-blocking mode, it will return any data that the system has in it's read buffer for that socket.
 * But, it won't wait for that data.
 * If the read buffer is empty, the system will return from ::read() immediately saying "Operation Would Block!".
 * To be more precise ::read() returns -1 to indicate that an error occurred.
 * And errno is set to EAGAIN or EWOULDBLOCK.
 * errno as well as the macros EAGAIN and EWOULDBLOCK are defined in header <cerrno>
 * 
 * The "non-blocking" mode is set by changing one of the socket's "flags". The flags are a series of bits, each one representing a different capability of the socket.
 ...
 * @return true if mode could be set. false if the method failed to set the mode. On error you should check the errno value.
 */
...
/**
 * Once connection requests are enabled on a server socket, the select function reports when the socket has a connection ready to be accepted.
 */
static int select(int socket)
{
  ...
  // timeout must be set each time select is called because select updates the timeout argument to indicate how much time was left.
  ...
  // A struct timeval represents a time as a number of seconds (tv_sec) plus a number of microseconds (tv_usec) between 0 and 999,999. Thus, to represent 10 milliseconds, you would use 10,000 microseconds.
  timeval timeout = {};
  timeout.tv_sec = 0;
  ...
  // The sets are modified by select() in place to indicate which file descriptors actually changed status.
  ...
  // The number nfds is the highest-numbered file descriptor in any of the three sets, plus 1.
  // Here we use just 1 socket.
  // If you don't know the highest number of sockets beforehand use FD_SETSIZE which is macro defined by system that specifies the max number of file descriptors of an fd_set
  int nfds = FD_SETSIZE;
  ...
  // On success, select() returns the number of file descriptors contained in the three returned descriptor sets (that is, the total number of bits that are set in readfds, writefds, exceptfds)
  // which may be zero if the timeout expires before anything interesting happens.
  // On error, -1 is returned, and errno is set appropriately;
  // the sets and timeout become undefined, so do not rely on their contents after an error.
  int result = ::select(nfds, &readfds, NULL /*&writefds*/, NULL /*&exceptfds*/, &timeout);
  ...
  // FD_ISSET returns a nonzero value (true) if filedes/socket is a member of the file descriptor set, and zero (false) otherwise.
 ...
 * The server’s original socket does not become part of the connection.
 * Instead, accept makes a new socket which participates in the connection.
 * accept returns the descriptor for this socket.
 * The server’s original socket remains available for listening for further connection requests.
 * 
 ...
 * 
 * @return On success, a nonnegative integer that is a file descriptor for the accepted socket. On error, -1 is returned, errno is set appropriately
 */
...
bool Server::open()
{
  ...
  // Allow socket descriptor to be reuseable (Forecefully attaching socket to the port)
...
void Server::run()
{
  ...
  while (this->enabled)
  {
    ...
    if (rc == -1)
    {
      // an error occurred or the connection has been closed before accept could be executed
      std::cerr << "Failed to accept." << std::endl;
      printError();
      // continue. The select block will handle the termination if the socket in not valid .
      continue;
    }
    else if (!this->enabled)
    {
    ...
    {
      std::cerr << "Failed to set client socket in non blocking mode" << std::endl;
      printError();
      continue;
    }
